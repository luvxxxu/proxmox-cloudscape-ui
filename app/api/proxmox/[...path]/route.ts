...
import { proxmoxRequestBody } from '@/app/lib/proxmox-request-body';
...
async function proxyRequest(request: NextRequest, { params }: Context) {
  ...
  try {
    ...
    const signal = requestTimeout(request, Number.isFinite(timeout) && timeout > 0 ? timeout : 60000);
    const options: RequestInit & { duplex?: 'half' } = {
      method: request.method, headers, cache: 'no-store', redirect: 'manual',
      signal,
    };
    if (!['GET', 'HEAD'].includes(request.method)) {
      const framed = await proxmoxRequestBody(request, headers, signal);
      if (framed instanceof Response) return framed;
      Object.assign(options, framed);
    }
