...
export async function POST(request: NextRequest) {
  ...
  const endpoint = vmtype;
  const proxyEndpoint = getConsoleProxyEndpoint(endpoint, mode);
  ...
  const consoleRes = await fetch(
    `${PROXMOX_HOST}/api2/json/nodes/${encodeURIComponent(node)}/${endpoint}/${encodeURIComponent(String(vmid))}/${proxyEndpoint}`,
    {
      method: "POST",
      headers,
  ...
  if (!consoleRes.ok) {
    ...
    if (mode === "xterm" && endpoint === "qemu" && /serial interface/i.test(text)) {
      return Response.json({
        code: "SERIAL_INTERFACE_REQUIRED",
        error: "xterm.js requires a configured QEMU serial interface",
      }, { status: 409 });
    }
