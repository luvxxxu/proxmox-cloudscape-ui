#!/usr/bin/env bash
set -euo pipefail
# Real Debian packages, verified runtimes, application build and HTTPS services.
...
[[ -f "$repo_dir/build/proxmox-cloudscape-ui-source.tar.gz" ]] || { echo 'Run bun run package:source first.' >&2; exit 1; }
...
case "$action:$service" in
  ...
  restart:proxmox-cloudscape|start:proxmox-cloudscape|stop:proxmox-cloudscape)
    ...
    setpriv --reuid=proxmox-ui --regid=proxmox-ui --init-groups /usr/local/bin/node server/custom-server.js 9>&- >> /tmp/fixture-app.log 2>&1 &
    printf '%s\n' "$!" > /run/fixture-app.pid ;;
...
cd /root/source
install_args=(--proxmox-host https://localhost:18006 --app-origin https://ui.test \
...
printf '\nPASS: genuine Debian install, verified downloads, non-root build, HTTPS, API denial, file permissions\n'

# Reuse the verified build only for repeated deployment/fault scenarios. Production
# install-lxc.sh always runs install/check/audit/build; it has no skip-check option.
cp -a "$first" /var/lib/proxmox-ui-build/verified-build
chown -R proxmox-ui-build:proxmox-ui-build /var/lib/proxmox-ui-build/verified-build
mv /opt/proxmox-cloudscape-tools/bin/bun /opt/proxmox-cloudscape-tools/bin/bun-real
cat > /opt/proxmox-cloudscape-tools/bin/bun <<'REUSE_BUILD'
#!/bin/bash
set -euo pipefail
case "$*" in
  --version) echo 1.3.12 ;;
  install*|run\ check|audit) exit 0 ;;
  run\ build) cp -a /var/lib/proxmox-ui-build/verified-build/. . ;;
  *) exit 1 ;;
esac
REUSE_BUILD
chmod 0755 /opt/proxmox-cloudscape-tools/bin/bun
bash deploy/install-lxc.sh "${install_args[@]}"
